@php
    ...
    if (session('peran') == 'admin') {
        $menu = [
            ...
            ['route' => 'admin-data-alumni', 'label' => 'Data Alumni', 'icon' => 'fa-solid fa-list'],
            ['route' => 'log-admin', 'label' => 'Log', 'icon' => 'fa-solid fa-list'],
        ];
        $logout = 'logout-admin';
    } elseif (session('peran') == 'admin-manager') {
        $menu = [
            ...
            <div class="text-lg">
                @foreach ($menu as $menu)
                    <li><a href="{{ route($menu['route']) }}"
                            ...
                            <i class="{{ $menu['icon'] }} inline-flex text-center min-w-6 mr-2"></i>
                            {{ $menu['label'] }}</a>
                    </li>
                @endforeach
                <li class="flex flex-row items-center">
                    <form action="{{ route($logout) }}" method="POST">
                        @csrf
                        <button type="submit"><i class="fa-solid fa-right-from-bracket inline-flex text-center min-w-6 mr-2"></i> Logout</button>
                    </form>
                </li>
            </div>
        </ul>
