...
@php
    $menu = [
        ...
        ['route' => 'data-alumni', 'label' => 'Data Alumni', 'icon' => 'fa-solid fa-list'],
...
<div class="fixed top-0 navbar bg-base-100 border-b border-neutral-content z-10">
    ...
    <div class="navbar-end">
        <label for="my-drawer" class="flex flex-row items-center cursor-pointer mr-6">
                ...
                @foreach ($menu as $menu)
                    <li><a href="{{ route($menu['route']) }}"
                            class="{{ Route::is($menu['route']) ? 'text-primary' : '' }}"><i
                                class="{{ $menu['icon'] }} inline-flex text-center min-w-6 mr-2"></i> {{ $menu['label'] }}</a>
                    </li>
                @endforeach
            </div>
        </ul>
